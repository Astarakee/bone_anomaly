...
from libs import self_inpaint_kl
...
from libs.batch_test_new_kl import create_mast_from_kldist, binarize_label
from libs.paths_dirs_stuff import get_data_list, get_sub_dirs, creat_dir
from libs.batch_test_new_kl import create_mast_from_level_set_seg, save_normailzed_color_image
...
def auto_inpaint_kl(write_path_results, exp_name, img_path, field_path, \
        ...
        for ix in range(img_array.shape[0]):
            ...
            if field_sum_int>50: # instead of 0 avoid tiny colon ...
                ...
                for iteration in range(0, 3):
                    posterior_mean, posterior_std = self_inpaint_kl.create_uncertainty_map(deepcopy(fixed_image),
                                                                                        grid_size,
                                                                                        overlap_factor,
                    ...
                    kl_dist = self_inpaint_kl.kullback_leible_distance_gaussian(posterior_mean, posterior_std, prior_mean, prior_std)
                    ...
                    fixed_image = self_inpaint_kl.inpaint_with_mask(deepcopy(main_img), cur_mask, model=model)

                prior_mean, prior_std = self_inpaint_kl.create_uncertainty_map(deepcopy(fixed_image),
                                                                            grid_size,
                                                                            overlap_factor,
                                                                            batch_size=batch_size,
                                                                            model=model,
                ...
                entropy_gain = self_inpaint_kl.entropy_gain_gaussian(posterior_std,prior_std)
                ...
                for iteration in range(1, num_iteration):
                    ...
                    fixed_image = self_inpaint_kl.inpaint_with_mask(deepcopy(main_img), mask, model=model)
                    attempt_mean, attempt_std = self_inpaint_kl.create_uncertainty_map(deepcopy(fixed_image),
                                                                                    grid_size,
                                                                                    overlap_factor,
                                                                                    batch_size=batch_size,
                                                                                    model=model,
                    ...
                    kl_dist_attempt = self_inpaint_kl.kullback_leible_distance_gaussian(attempt_mean, attempt_std, prior_mean, prior_std)

                    entropy_gain = self_inpaint_kl.entropy_gain_gaussian(posterior_std, attempt_std)
