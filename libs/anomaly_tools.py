...
def get_circle_candids(random_masks,main_img, n_top_circle):
    mean_val_ = []
    max_val_ = []
    std_val_ = []
    circle_candid_id = []
    for ind in range(len(random_masks)):
        ...
        rand_circle = random_masks[ind]
        rand_circle = 1-rand_circle
        
        img_circle = main_img*rand_circle
    ...
    for ix in top_mean_val:
        corres_max = max_val_[ix]
        if corres_max == 1:
            circle_candid_id.append(ix)
